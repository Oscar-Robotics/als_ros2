...
class SLAMER : public MCL
{
  ...
  public:
  // TODO: figure out how to use the Node constructor
    SLAMER(std::string ismYamlFile)
        : /*Node("slamer"),*/ ismPointsName_("/ism_points"), coloredScanPointsName_("/slamer_colored_scan_points"),
          lineObjectsName_("/slamer_line_objects"), spatialLineObjectsName_("/slamer_spatial_line_objects"),
          publishISMPoints_(true), publishColoredScanPoints_(true)
    {
        // slamer parameters
        this->declare_parameter("ism_points_name", ismPointsName_);
        this->declare_parameter("colored_scan_points_name", coloredScanPointsName_);
        this->declare_parameter("line_objects_name", lineObjectsName_);
        this->declare_parameter("spatial_line_objects_name", spatialLineObjectsName_);
        this->declare_parameter("publish_ism_points", publishISMPoints_);
        this->declare_parameter("publish_colored_scan_points", publishColoredScanPoints_);
        ...
        this->declare_parameter("scan_range_max", 20.0);
        this->declare_parameter("scan_map_reso", 0.05);
        this->declare_parameter("scan_orientation_hist_reso", 5.0);
        this->declare_parameter("min_scan_orientation_prob", 0.1);
        this->declare_parameter("min_spatial_line_object_length", 0.5);
        this->declare_parameter("max_spatial_line_object_length", 2.5);
        this->declare_parameter("merge_points_dist", 0.5);
        ...
        recog_.init(scanRangeMax, scanMapReso, scanOrientationHistReso, minScanOrientationProb,
                    minSpatialLineObjectLength, maxSpatialLineObjectLength, mergePointsDist);
    }

    void calculateLikelihoodsBySLAMER(void)
    {
        scan_ = getScan();
        recog_.recognizeLineObjects(scan_, lineObjects_, lineObjectsProbs_, spatialLineObjects_,
                                    spatialLineObjectsProbs_);
        if ((int)lineObjects_.size() == 0)
        {
            RCLCPP_INFO(get_logger(),
                        "Likelihood calculation with SLAMER is ignored since actual line objects were not detected.");
            return;
        }
        ...
        for (int i = 0; i < (int)lineObjects_.size(); ++i)
        {
            ...
            for (int j = 0; j < particlesNum; ++j)
            {
                ...
                double pCloseDoor =
                    calculateLineObjectClassificationModel(lineObjectsProbs_[i], (int)LineObjectLabel::CLOSE_DOOR) *
                    lineObjectPrior[(int)LineObjectLabel::CLOSE_DOOR];
                double pCloseGlassDoor = calculateLineObjectClassificationModel(
                                             lineObjectsProbs_[i], (int)LineObjectLabel::CLOSE_GLASS_DOOR) *
                                         lineObjectPrior[(int)LineObjectLabel::CLOSE_GLASS_DOOR];
                double pFence =
                    calculateLineObjectClassificationModel(lineObjectsProbs_[i], (int)LineObjectLabel::FENCE) *
                    lineObjectPrior[(int)LineObjectLabel::FENCE];
                double pOthers =
                    calculateLineObjectClassificationModel(lineObjectsProbs_[i], (int)LineObjectLabel::OTHERS) *
                    lineObjectPrior[(int)LineObjectLabel::OTHERS];
    ...
    void recognizeObjectsWithMapAssist(void)
    {
        ...
        for (int i = 0; i < (int)lineObjects_.size(); ++i)
        {
            ...
            double pCloseDoor =
                calculateLineObjectClassificationModel(lineObjectsProbs_[i], (int)LineObjectLabel::CLOSE_DOOR) *
                lineObjectPrior[(int)LineObjectLabel::CLOSE_DOOR];
            double pCloseGlassDoor =
                calculateLineObjectClassificationModel(lineObjectsProbs_[i], (int)LineObjectLabel::CLOSE_GLASS_DOOR) *
                lineObjectPrior[(int)LineObjectLabel::CLOSE_GLASS_DOOR];
            double pFence = calculateLineObjectClassificationModel(lineObjectsProbs_[i], (int)LineObjectLabel::FENCE) *
                            lineObjectPrior[(int)LineObjectLabel::FENCE];
            double pOthers =
                calculateLineObjectClassificationModel(lineObjectsProbs_[i], (int)LineObjectLabel::OTHERS) *
                lineObjectPrior[(int)LineObjectLabel::OTHERS];
        ...
        for (int i = 0; i < (int)spatialLineObjects_.size(); ++i)
        {
            ...
            double pOpenDoor = calculateSpatialLineObjectClassificationModel(spatialLineObjectsProbs_[i],
                                                                             (int)LineObjectLabel::OPEN_DOOR) *
                               spatialLineObjectPrior[(int)LineObjectLabel::OPEN_DOOR];
            double pOpenGlassDoor = calculateSpatialLineObjectClassificationModel(
                                        spatialLineObjectsProbs_[i], (int)LineObjectLabel::OPEN_GLASS_DOOR) *
                                    spatialLineObjectPrior[(int)LineObjectLabel::OPEN_GLASS_DOOR];
            double pCloseGlassDoor = calculateSpatialLineObjectClassificationModel(
                                         spatialLineObjectsProbs_[i], (int)LineObjectLabel::CLOSE_GLASS_DOOR) *
                                     spatialLineObjectPrior[(int)LineObjectLabel::CLOSE_GLASS_DOOR];
            double pFence = calculateSpatialLineObjectClassificationModel(spatialLineObjectsProbs_[i],
                                                                          (int)LineObjectLabel::FENCE) *
                            spatialLineObjectPrior[(int)LineObjectLabel::FENCE];
            double pNoEntryLine = calculateSpatialLineObjectClassificationModel(spatialLineObjectsProbs_[i],
                                                                                (int)LineObjectLabel::NO_ENTRY) *
                                  spatialLineObjectPrior[(int)LineObjectLabel::NO_ENTRY];
            double pFreeSpace = calculateSpatialLineObjectClassificationModel(spatialLineObjectsProbs_[i],
                                                                              (int)LineObjectLabel::FREE_SPACE) *
                                spatialLineObjectPrior[(int)LineObjectLabel::FREE_SPACE];
            double pOthers = calculateSpatialLineObjectClassificationModel(spatialLineObjectsProbs_[i],
                                                                           (int)LineObjectLabel::OTHERS) *
                             spatialLineObjectPrior[(int)LineObjectLabel::OTHERS];
    ...
    void publishSLAMERROSMessages(void)
    {
        if (publishColoredScanPoints_)
        {
            sensor_msgs::msg::PointCloud2 coloredScanImgPoints =
                recog_.getColoredScanImgPoints(lineObjects_, lineObjectsProbs_);
        ...
        for (int i = 0; i < (int)spatialLineObjects_.size(); ++i)
        {
            ...
            // printf("%d: %lf, %lf, %lf, %lf, %lf, %lf %lf\n", i, pOpenDoor, pOpenGlassDoor, pFence, pCloseGlassDoor,
            // pNoEntryLine, pFreeSpace, pOthers);

            std_msgs::msg::ColorRGBA color;
            color.a = 0.5;
            if (pFreeSpace > pOpenDoor && pFreeSpace > pOpenGlassDoor && pFreeSpace > pCloseGlassDoor &&
                pFreeSpace > pFence && pFreeSpace > pNoEntryLine && pFreeSpace > pOthers)
            {
                // recog_.getObjectColor((int)LineObjectLabel::FREE_SPACE, color.r, color.g, color.b);
                continue;
            }
            else if (pOthers > pOpenDoor && pOthers > pOpenGlassDoor && pOthers > pCloseGlassDoor && pOthers > pFence &&
                     pOthers > pNoEntryLine)
            {
                // recog_.getObjectColor((int)LineObjectLabel::OTHERS, color.r, color.g, color.b);
                continue;
            }
            else if (pOpenDoor < 0.5 && pOpenGlassDoor < 0.5 && pCloseGlassDoor < 0.5 && pFence < 0.5 &&
                     pNoEntryLine < 0.5)
            {
                continue;
            }
            else if (pOpenDoor > pOpenGlassDoor && pOpenDoor > pCloseGlassDoor && pOpenDoor > pFence &&
                     pOpenDoor > pNoEntryLine)
            {
  ...
  private:
    ...
    double calculateSpatialLineObjectClassificationModel(std::vector<double> classificationProbs, int objectID)
    {
        ...
        if (objectID == (int)LineObjectLabel::OPEN_DOOR)
            coefs[0] = coef1, coefs[1] = coef2, coefs[2] = coef2, coefs[3] = coef2, coefs[4] = coef2, coefs[5] = coef2,
            coefs[6] = coef2;
        else if (objectID == (int)LineObjectLabel::OPEN_GLASS_DOOR)
            coefs[0] = coef2, coefs[1] = coef1, coefs[2] = coef2, coefs[3] = coef2, coefs[4] = coef2, coefs[5] = coef2,
            coefs[6] = coef2;
        else if (objectID == (int)LineObjectLabel::CLOSE_GLASS_DOOR)
            coefs[0] = coef2, coefs[1] = coef2, coefs[2] = coef1, coefs[3] = coef2, coefs[4] = coef2, coefs[5] = coef2,
            coefs[6] = coef2;
        else if (objectID == (int)LineObjectLabel::FENCE)
            coefs[0] = coef2, coefs[1] = coef2, coefs[2] = coef2, coefs[3] = coef1, coefs[4] = coef2, coefs[5] = coef2,
            coefs[6] = coef2;
        else if (objectID == (int)LineObjectLabel::NO_ENTRY)
            coefs[0] = coef2, coefs[1] = coef2, coefs[2] = coef2, coefs[3] = coef2, coefs[4] = coef1, coefs[5] = coef2,
            coefs[6] = coef2;
        else if (objectID == (int)LineObjectLabel::FREE_SPACE)
            coefs[0] = coef2, coefs[1] = coef2, coefs[2] = coef2, coefs[3] = coef2, coefs[4] = coef2, coefs[5] = coef1,
            coefs[6] = coef2;
        else if (objectID == (int)LineObjectLabel::OTHERS)
            coefs[0] = coef2, coefs[1] = coef2, coefs[2] = coef2, coefs[3] = coef2, coefs[4] = coef2, coefs[5] = coef2,
            coefs[6] = coef1;
    ...
    std::vector<double> getLineObjectPrior(Pose sensorPose, LineObject lineObject)
    {
        ...
        for (double l = 0.0; l <= dl; l += mapResolution_)
        {
            ...
            else
            {
                ...
                lineObjectPrior[(int)LineObjectLabel::CLOSE_DOOR] =
                    clipProb(lineObjectPrior[(int)LineObjectLabel::CLOSE_DOOR]);
                lineObjectPrior[(int)LineObjectLabel::CLOSE_GLASS_DOOR] =
                    clipProb(lineObjectPrior[(int)LineObjectLabel::CLOSE_GLASS_DOOR]);
    ...
    std::vector<double> getSpatialLineObjectPrior(Pose sensorPose, LineObject spatialLineObject)
    {
        ...
        for (double l = 0.0; l <= dl; l += mapResolution_)
        {
            ...
            float othersDist = ism_.getDistance(xx1, yy1, "others");
            if (doorDist < 0.0f && glassDoorDist < 0.0f && fenceDist < 0.0 && doNotEnterLineDist < 0.0 &&
                othersDist < 0.0)
            {
            ...
            else
            {
                ...
                spatialLineObjectPrior[(int)LineObjectLabel::OPEN_DOOR] =
                    clipProb(spatialLineObjectPrior[(int)LineObjectLabel::OPEN_DOOR]);
                spatialLineObjectPrior[(int)LineObjectLabel::OPEN_GLASS_DOOR] =
                    clipProb(spatialLineObjectPrior[(int)LineObjectLabel::OPEN_GLASS_DOOR]);
                spatialLineObjectPrior[(int)LineObjectLabel::OPEN_GLASS_DOOR] =
                    clipProb(spatialLineObjectPrior[(int)LineObjectLabel::OPEN_GLASS_DOOR]);
                spatialLineObjectPrior[(int)LineObjectLabel::FENCE] =
                    clipProb(spatialLineObjectPrior[(int)LineObjectLabel::FENCE]);
                spatialLineObjectPrior[(int)LineObjectLabel::NO_ENTRY] =
                    clipProb(spatialLineObjectPrior[(int)LineObjectLabel::NO_ENTRY]);
                spatialLineObjectPrior[(int)LineObjectLabel::FREE_SPACE] =
                    clipProb(spatialLineObjectPrior[(int)LineObjectLabel::FREE_SPACE]);
                spatialLineObjectPrior[(int)LineObjectLabel::OTHERS] =
                    clipProb(spatialLineObjectPrior[(int)LineObjectLabel::OTHERS]);
                ...
                /*
                                ...
                                printf("%lf, %lf, %lf, %lf\n", spatialLineObjectPrior[(int)LineObjectLabel::OPEN_DOOR],
                   spatialLineObjectPrior[(int)LineObjectLabel::FENCE],
                                    spatialLineObjectPrior[(int)LineObjectLabel::UNOBSERVABLE_WALL],
                   spatialLineObjectPrior[(int)LineObjectLabel::FREE_SPACE]); printf("\n");
                 */
            }
            xx1 += ddx;
            yy1 += ddy;
