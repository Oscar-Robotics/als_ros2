...
class MAEClassifier
{
  ...
  public:
    MAEClassifier(void) : maxResidualError_(1.0), maeHistogramBinWidth_(0.01)
    {
    }

    inline void setClassifierDir(std::string classifiersDir)
    {
        classifiersDir_ = classifiersDir;
    }
    inline void setMaxResidualError(double maxResidualError)
    {
        maxResidualError_ = maxResidualError;
    }
    inline void setMAEHistogramBinWidth(double maeHistogramBinWidth)
    {
        maeHistogramBinWidth_ = maeHistogramBinWidth;
    }

    inline double getFailureThreshold(void)
    {
    ...
    void learnThreshold(std::vector<std::vector<double>> trainSuccessResidualErrors,
    ...
    void writeClassifierParams(std::vector<std::vector<double>> testSuccessResidualErrors,
    ...
    double calculateDecisionModel(double mae, double *reliability)
    {
        ...
        pSuccess = dTruePositive_ * positiveMAEHistogram_.getProbability(mae) +
                   dFalsePositive_ * positiveMAEHistogram_.getProbability(mae);
        pFailure = dTrueNegative_ * negativeMAEHistogram_.getProbability(mae) +
                   dFalsePositive_ * negativeMAEHistogram_.getProbability(mae);
        if (pSuccess < 10.0e-9)
            pSuccess = 10.0e-9;
        if (pFailure < 10.0e-9)
            pFailure = 10.0e-9;
