...
class LineObject
{
  ...
  public:
    LineObject(void) : p1_(als_ros::Point(0.0, 0.0)), p2_(als_ros::Point(0.0, 0.0)), scanPointsNum_(0)
    {
    }

    LineObject(double x1, double y1, double x2, double y2, int scanPointsNum)
        : p1_(als_ros::Point(x1, y1)), p2_(als_ros::Point(x2, y2)), scanPointsNum_(scanPointsNum)
    {
    }
    ...
    inline als_ros::Point getP1(void)
    {
        return p1_;
    }
    inline als_ros::Point getP2(void)
    {
        return p2_;
    }
    inline int getScanPointsNum(void)
    {
        return scanPointsNum_;
    }
...
class LineObjectRecognition
{
  ...
  public:
    ...
    void recognizeLineObjects(sensor_msgs::msg::LaserScan scan, std::vector<LineObject> &lineObjects,
                              std::vector<std::vector<double>> &lineObjectsProbs,
                              std::vector<LineObject> &spatialLineObjects,
                              std::vector<std::vector<double>> &spatialLineObjectsProbs)
    {
    ...
    sensor_msgs::msg::PointCloud2 getColoredScanImgPoints(std::vector<LineObject> lineObjects,
                                                          std::vector<std::vector<double>> lineObjectsProbs)
    {
  ...
  private:
    ...
    std::vector<double> getScanOrientationHistogram(std::vector<cv::Vec4i> scanLines)
    {
        ...
        for (int i = 0; i < scanOrientationHistBinMax_; ++i)
        {
            ...
            // printf("i = %d, rad = %lf, val = %lf (%d)\n", i, (double)i * scanOrientationHistReso_, hist[i],
            // scanLineSize);
        }
        return hist;
    }

    std::vector<LineObject> makeLineObjects(cv::Mat scanImg, std::vector<cv::Vec4i> scanLines,
                                            std::vector<double> scanOriHist)
    {
    ...
    std::vector<LineObject> makeSpatialLineObjects(cv::Mat scanImg, std::vector<als_ros::Point> scanImgPoints,
                                                   std::vector<double> scanOriHist)
    {
        std::vector<LineObject> spatialLineObjects;
    ...
    std::vector<LineObject> checkScanPassability(std::vector<LineObject> spatialLineObjects,
                                                 std::vector<als_ros::Point> scanImgPoints)
    {
        ...
        for (;;)
        {
            ...
            for (int i = 0; i < (int)lineObjects.size(); ++i)
            {
                ...
                if (dl1 < mergePointsDist_ && dl2 < mergePointsDist_)
                {
                    ...
                    mergedLineObjects[targetIdx].setLineObject(als_ros::Point(x1, y1), als_ros::Point(x2, y2),
                                                               scanPointsNum);
    ...
    void printLineObjects(std::vector<LineObject> lineObjects, std::string type)
    {
        for (int i = 0; i < (int)lineObjects.size(); ++i)
        {
            ...
            printf("type = %s, x1 = %lf, y1 = %lf, x2 = %lf, y2 = %lf, scanPointsNum = %d\n", type.c_str(), p1.getX(),
                   p1.getY(), p2.getX(), p2.getY(), scanPointsNum);
        }
        printf("\n");
    }
