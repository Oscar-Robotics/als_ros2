...
class ISM
{
  ...
  public:
    ISM() : ismPoints_(new pcl::PointCloud<pcl::PointXYZRGB>())
    {
    }

    inline std::vector<std::string> getObjectNames(void)
    {
        return objectNames_;
    }
    inline double getMapResolution(void)
    {
    ...
    void readISM(std::string ismYamlFile)
    {
        ...
        if (ismImage.empty())
        {
            // Raise exception for file not found
            throw std::runtime_error("Unable to open image file: " + ismFilePath);
        }
        ...
        for (int i = 0; i < (int)objectNames_.size(); ++i)
        {
            ...
            // std::cout << i << " " << objectColors_[i][0] << " " << objectColors_[i][1] << " " << objectColors_[i][2]
            // << std::endl;
        }
  ...
  private:
    template <typename T> inline void xy2uv(T x, T y, int *u, int *v)
    {
        *u = (int)((x - mapOrigin_.getX()) / mapResolution_);
        *v = mapHeight_ - 1 - (int)((y - mapOrigin_.getY()) / mapResolution_);
    }

    template <typename T> inline void uv2xy(int u, int v, T *x, T *y)
    {
