...
class ClassifierDatasetGenerator : public rclcpp::Node
{
  ...
  public:
    ClassifierDatasetGenerator(void)
        : Node("classifier_dataset_generator", rclcpp::NodeOptions()), mapName_("/map"), generateSampleNum_(1000),
          saveDir_("/tmp/classifier_dataset/"), obstaclesNum_(20), angleMin_(-135.0), angleMax_(135.0),
          angleIncrement_(0.25), rangeMin_(0.02), rangeMax_(30.0), scanAngleNoise_(0.01), scanRangeNoise_(0.05),
          validScanRateTH_(0.1), failurePositionalErrorTH_(0.2), failureAngularErrorTH_(2.0), positionalErrorMax_(0.5),
          angularErrorMax_(5.0), gotMap_(false)
    {
    }
    ...
    void generateDataset(void)
    {
        for (int i = 0; i < generateSampleNum_; ++i)
        {
            ...
            if (!isValidScan(scan))
            {
                RCLCPP_INFO(get_logger(), "Simulated scan is invalid.");
                i--;
                continue;
            }
            ...
            if ((i + 1) % 10 == 0)
                RCLCPP_INFO(get_logger(), "%.2lf [%%] process done.",
    ...
    {
        readDataset(trainDirs_, gtPoses, successPoses, failurePoses, scans, successResidualErrors,
    ...
    void readTestDataset(std::vector<Pose> &gtPoses, std::vector<Pose> &successPoses, std::vector<Pose> &failurePoses,
                         ...
                         std::vector<std::vector<double>> &failureResidualErrors)
    {
        readDataset(testDirs_, gtPoses, successPoses, failurePoses, scans, successResidualErrors,
                    failureResidualErrors);
    }
  ...
  private:
    ...
    void generatePoses(Pose &gtPose, Pose &successPose, Pose &failurePose)
    {
        ...
        for (;;)
        {
            ...
            if (fabs(x) >= failurePositionalErrorTH_ || fabs(y) >= failureAngularErrorTH_ ||
    ...
    void saveData(Pose gtPose, Pose successPose, Pose failurePose, sensor_msgs::msg::LaserScan scan,
                  std::vector<double> successResidualErrors, std::vector<double> failureResidualErrors)
    {
    ...
    void readDataset(std::vector<std::string> dirs, std::vector<Pose> &gtPoses, std::vector<Pose> &successPoses,
                     std::vector<Pose> &failurePoses, std::vector<sensor_msgs::msg::LaserScan> &scans,
                     std::vector<std::vector<double>> &successResidualErrors,
                     std::vector<std::vector<double>> &failureResidualErrors)
    {
        FILE *fp;
