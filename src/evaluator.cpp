...
class Evaluator : public rclcpp::Node
{
  ...
  public:
    Evaluator(void)
        : Node("als_ros_evaluator"), mapFrame_("map"), laserFrame_("laser"), canUpdateGTPoses_(true),
          canUpdateGLPoses_(true), canUpdateScan_(true)
    {
        gtPoseSub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
            "/scanned_ground_truth_pose", 1, [this](geometry_msgs::msg::PoseStamped::SharedPtr msg) {
                this->gtPoseCB(msg);
            });
        reliabilitySub_ = this->create_subscription<geometry_msgs::msg::Vector3Stamped>(
            "/reliability", 1, [this](geometry_msgs::msg::Vector3Stamped::SharedPtr msg) {
                this->reliabilityCB(msg);
            });
        glPosesSub_ = this->create_subscription<geometry_msgs::msg::PoseArray>(
            "/gl_sampled_poses", 1, [this](geometry_msgs::msg::PoseArray::SharedPtr msg) {
                this->glPosesCB(msg);
            });
        scanSub_ =
            this->create_subscription<sensor_msgs::msg::LaserScan>("/scan", 1, [this](sensor_msgs::msg::LaserScan::SharedPtr msg) {
                this->scanCB(msg);
            });
    ...
    void reliabilityCB(const geometry_msgs::msg::Vector3Stamped::SharedPtr &msg)
    {
        ...
        try
        {
            ...
            tfMap2Laser = tfBuffer_->lookupTransform(mapFrame_, laserFrame_, tf2_time);
        }
        ...
        if (saveGTPose_)
        {
            tf2::Quaternion gtQuat(gtPose.pose.orientation.x, gtPose.pose.orientation.y, gtPose.pose.orientation.z,
        ...
        auto rotation = tfMap2Laser.transform.rotation;
        auto translation = tfMap2Laser.transform.translation;
        ...
        if (saveGTPose_)
        {
            ...
            while (dyaw > M_PI)
                dyaw -= 2.0 * M_PI;
            printf("%.2lf [sec], %.3lf [m], %.3lf [m], %.3lf [m], %.3lf [deg], %lf %lf %lf\n", time - firstTime, dx, dy,
                   dl, dyaw * 180.0 / M_PI, msg->vector.x, msg->vector.y, msg->vector.z);

            fprintf(fp, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", time - firstTime, gtX, gtY, gtYaw,
                    x, y, yaw, dx, dy, dl, dyaw, msg->vector.x, msg->vector.y, msg->vector.z);
        }
        else
        {
            printf("%.2lf [sec], %.3lf [m], %.3lf [m], %.3lf [deg], %lf %lf %lf\n", time - firstTime, x, y,
                   yaw * 180.0 / M_PI, msg->vector.x, msg->vector.y, msg->vector.z);

            fprintf(fp, "%lf %lf %lf %lf %lf %lf %lf\n", time - firstTime, x, y, yaw, msg->vector.x, msg->vector.y,
                    msg->vector.z);
        }
        ...
        if (saveGLPoses_)
        {
            ...
            {
                tf2::Quaternion quat(glPoses.poses[i].orientation.x, glPoses.poses[i].orientation.y,
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<rclcpp::Node>("als_ros_evaluator");
